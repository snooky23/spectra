# Documentation Index

This folder contains all Spectra Logger documentation organized by category.

## 📚 User Guides (`guides/`)
Documentation for integrating and using Spectra Logger:
- [Installation Guide](guides/INSTALLATION.md) - Get started with Spectra
- [Usage Guide](guides/USAGE_GUIDE.md) - How to use the SDK
- [API Reference](guides/API.md) - Complete API documentation
- [iOS Testing Guide](guides/iOS_TESTING_GUIDE.md) - Testing on iOS

### Package Manager Guides
- [Swift Package Manager](guides/SPM_DISTRIBUTION_GUIDE.md)
- [Swift Package Setup](guides/SWIFT_PACKAGE_SETUP.md)

## 🎨 Design (`design/`)
Product and UI design documentation:
- [UI Design Specification](design/UI_DESIGN.md) - Screen layouts and components
- [Architecture](design/ARCHITECTURE.md) - Technical architecture
- [Product Requirements](design/PRD.md) - Full PRD
- [PRD Changes](design/PRD_CHANGES.md) - Change log for PRD

## 🚀 Release (`release/`)
Publishing and release documentation:
- [Changelog](release/CHANGELOG.md) - Version history
- [Release Process](release/RELEASE_PROCESS.md) - How to release
- [Publishing Guide](release/PUBLISHING.md) - Maven/SPM publishing
- [Version Management](release/VERSION_MANAGEMENT.md) - Versioning strategy
- [CI/CD Setup](release/CICD_SETUP.md) - GitHub Actions setup
- [Package Distribution](release/PACKAGE_DISTRIBUTION_GUIDE.md)
- [v1.0.0 Release Instructions](release/RELEASE_v1.0.0_INSTRUCTIONS.md)

## 🔧 Internal (`internal/`)
Development session and planning docs (for contributors):
- [Planning](internal/PLANNING.md) - Project roadmap
- [Tasks](internal/TASKS.md) - Development tasks
- [Session Notes](internal/SESSION.md) - Dev session notes
- [CI Fixes](internal/CI_FIXES_COMPLETE.md) - CI troubleshooting notes
