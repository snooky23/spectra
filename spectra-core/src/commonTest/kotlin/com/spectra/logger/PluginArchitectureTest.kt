...
class PluginArchitectureTest {
    ...
    @Test
    fun testCustomLogSinkReceivesLogs() = runTest {
        SpectraLogger.setCoroutineScopeForTesting(this)
        val receivedEntries = mutableListOf<LogEntry>()
        val customSink = object : LogSink {
    ...
    @Test
    fun testCustomNetworkLogSinkReceivesLogs() = runTest {
        SpectraLogger.setCoroutineScopeForTesting(this)
        val receivedEntries = mutableListOf<NetworkLogEntry>()
        val customSink = object : NetworkLogSink {
    ...
    @Test
    fun testCrashingLogSinkDoesNotCrashHost() = runTest {
        SpectraLogger.setCoroutineScopeForTesting(this)
        var crashSinkCalled = false
        val crashingSink = object : LogSink {
            override suspend fun log(entry: LogEntry) {
