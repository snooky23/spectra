...
class InMemoryNetworkLogStorageTest {
    ...
    @Test
    fun testUpdateCapacity() =
        runTest {
            val storage = InMemoryNetworkLogStorage(maxCapacity = 5)
            (1..5).forEach { storage.add(createEntry(it.toString())) }
            assertEquals(5, storage.count())

            // Shrink capacity
            storage.updateCapacity(3)
            assertEquals(3, storage.count())
            val results = storage.query()
            assertEquals("5", results[0].id)
            assertEquals("4", results[1].id)
            ...
            // Grow capacity
            storage.updateCapacity(10)
            assertEquals(3, storage.count()) // Count shouldn't change
            storage.add(createEntry("6"))
